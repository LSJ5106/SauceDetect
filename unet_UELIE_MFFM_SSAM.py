...
class Unet(object):
    _defaults = {
        ...
        #-------------------------------------------------------------------#
        "model_path"    : 'logs_MFFM_UELIE_SSAM/best_weights.pth',
        #--------------------------------#
        #   所需要区分的类的个数+1
        #--------------------------------#
        "num_classes"   : 2,
    ...
    def get_miou_png(self, image):
        ...
        with torch.no_grad():
            ...
            # 为了让图片显色
            # pr_colored = np.zeros((orininal_h, orininal_w, 3), dtype=np.uint8)
            # color_map = {
            ...
            # }
            #
            # for i in range(orininal_h):
            #     for j in range(orininal_w):
            #         class_idx = pr[i, j]
